...
`rockykln.com`). The three quest projects shown on the page —
...
- Vanilla static site at `~/Schreibtisch/portfolio` — no build step,
  no framework.
- `wrangler.jsonc`, `_headers`, `_redirects` for Cloudflare Workers
  deploy.
...
- README + DESIGN + DEPLOY docs scaffolded.
